...
EntityCallbackUtils.setLogLevel(LogLevel.NONE);

export const basicLifeCycleTests = (context : RepositoryTestContext) : void => {

    const onPrePersistFn = jest.fn();
    const onPostPersistFn = jest.fn();
    const onPreRemoveFn = jest.fn();
    const onPostRemoveFn = jest.fn();
    const onPreUpdateFn = jest.fn();
    const onPostUpdateFn = jest.fn();
    const onPostLoadFn = jest.fn();
    ...
    beforeEach( async () => {
        ...
        onPrePersistFn.mockClear();
        onPostPersistFn.mockClear();
        onPreRemoveFn.mockClear();
        onPostRemoveFn.mockClear();
        onPreUpdateFn.mockClear();
        onPostUpdateFn.mockClear();
        onPostLoadFn.mockClear();
    ...
    describe('#count', () => {
        ...
        it('can count entities', async () => {
            ...
            expect( onPreUpdateFn ).not.toHaveBeenCalled();
    ...
    describe('#deleteAll', () => {
        ...
        it('can delete all entities', async () => {
            ...
            expect( onPreUpdateFn ).not.toHaveBeenCalled();
            expect( onPostUpdateFn ).not.toHaveBeenCalled();
            expect( onPostLoadFn ).toHaveBeenCalledTimes(4);
            expect( onPreRemoveFn ).toHaveBeenCalledTimes(4);
            expect( onPostRemoveFn ).toHaveBeenCalledTimes(4);
        ...
        it('can delete all entities with few ids', async () => {
            ...
            expect( onPreUpdateFn ).not.toHaveBeenCalled();
            expect( onPostUpdateFn ).not.toHaveBeenCalled();
            expect( onPostLoadFn ).toHaveBeenCalledTimes(2);
            expect( onPreRemoveFn ).toHaveBeenCalledTimes(2);
            expect( onPostRemoveFn ).toHaveBeenCalledTimes(2);
    ...
    describe('#save', () => {
        ...
        it('can save older entity', async () => {
            ...
            expect( onPrePersistFn ).not.toHaveBeenCalled();
            expect( onPostPersistFn ).not.toHaveBeenCalled();
            expect( onPreUpdateFn ).toHaveBeenCalledTimes(1);
        ...
        it('cannot save fresh entity if pre persist life cycle function throws', async () => {
            expect( await fooRepository.count() ).toBe(0);
            const newEntity = new FooEntity({fooName: 'Hello world', fooDate: '2023-05-12T15:42:09+03:00', nonUpdatable: 'hello'});
            onPrePersistFn.mockImplementationOnce(() => {
                throw new TypeError( 'onPrePersistFn' );
            });
            try {
                await fooRepository.save(newEntity);
            } catch (err) {
                expect(`${err}`).toContain('onPrePersistFn');
            }
            expect( await fooRepository.count() ).toBe(0);
            expect.assertions(3);
        });

        it('cannot save fresh entity if post persist life cycle function throws', async () => {
            expect( await fooRepository.count() ).toBe(0);
            const newEntity = new FooEntity({fooName: 'Hello world', fooDate: '2023-05-12T15:42:09+03:00', nonUpdatable: 'hello'});
            onPostPersistFn.mockImplementationOnce(() => {
                throw new TypeError( 'onPostPersistFn' );
            });
            try {
                await fooRepository.save(newEntity);
            } catch (err) {
                expect(`${err}`).toContain('onPostPersistFn');
            }
            expect( await fooRepository.count() ).toBe(0);
            expect.assertions(3);
        });

        it('cannot save older entity if PostUpdate life cycle method throws', async () => {

            expect( await barRepository.count() ).toBe(4);

            onPostUpdateFn.mockImplementationOnce(() => {
                throw new TypeError( 'onPostUpdateFn' );
            });

            barEntity2.barName = 'Hello world';

            try {
                await barRepository.save(barEntity2);
            } catch (err) {
                expect(`${err}`).toContain('onPostUpdateFn');
            }
    ...
    describe('#saveAll', () => {
        ...
        it('can save fresh entities', async () => {
            ...
            expect( onPreRemoveFn ).not.toHaveBeenCalled();
            expect( onPostRemoveFn ).not.toHaveBeenCalled();
            expect( onPrePersistFn ).toHaveBeenCalledTimes(2);
            expect( onPostPersistFn ).toHaveBeenCalledTimes(2);
            expect( onPostLoadFn ).toHaveBeenCalledTimes(2);
        ...
        it('can save older entities', async () => {
            ...
            expect( onPreUpdateFn ).toHaveBeenCalledTimes(2);
            expect( onPostUpdateFn ).toHaveBeenCalledTimes(2);
            expect( onPreRemoveFn ).not.toHaveBeenCalled();
            expect( onPostRemoveFn ).not.toHaveBeenCalled();
            expect( onPrePersistFn ).not.toHaveBeenCalled();
            expect( onPostPersistFn ).not.toHaveBeenCalled();
    ...
    describe('#deleteAllByBarDateBetween', () => {
        ...
        it('can delete all entities by barDate between range', async () => {
            ...
            expect( onPreUpdateFn ).not.toHaveBeenCalled();
            expect( onPostUpdateFn ).not.toHaveBeenCalled();
            expect( onPreRemoveFn ).toHaveBeenCalledTimes(3);
            expect( onPostRemoveFn ).toHaveBeenCalledTimes(3);
            expect( onPrePersistFn ).not.toHaveBeenCalled();
            expect( onPostPersistFn ).not.toHaveBeenCalled();
            expect( onPostLoadFn ).toHaveBeenCalledTimes(3);
    ...
    describe('#findByBarDateBefore', () => {
        ...
        it('cannot find an entity before there was any', async () => {
            ...
            expect( onPrePersistFn ).not.toHaveBeenCalled();
            expect( onPostPersistFn ).not.toHaveBeenCalled();
            expect( onPostLoadFn ).toHaveBeenCalledTimes(0);
