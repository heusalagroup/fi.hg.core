...
describe('parseArgumentWithParam', () => {
    ...
    describe('with built-in types', () => {
        it('parses boolean arguments correctly', () => {
            const result = parseArgumentWithParam('--foo=true', ArgumentType.BOOLEAN, '--foo', 'true', undefined);
            expect(result).toBe(true);
        });

        it('parses string arguments correctly', () => {
            const result = parseArgumentWithParam('--foo=bar', ArgumentType.STRING, '--foo', 'bar', undefined);
            expect(result).toBe('bar');
        });

        it('parses non-empty string arguments correctly', () => {
            const result = parseArgumentWithParam('--foo=bar', ArgumentType.NON_EMPTY_STRING, '--foo', 'bar', undefined);
            expect(result).toBe('bar');
        });

        it('parses number arguments correctly', () => {
            const result = parseArgumentWithParam('--foo=123', ArgumentType.NUMBER, '--foo', '123', undefined);
            expect(result).toBe(123);
        });

        it('parses integer arguments correctly', () => {
            const result = parseArgumentWithParam('--foo=123', ArgumentType.INTEGER, '--foo', '123', undefined);
            expect(result).toBe(123);
        });
    });
    ...
    describe('with custom type and parser', () => {
        it('uses the correct custom parser when provided', () => {
            ...
            const result = parseArgumentWithParam('--foo=bar', 'CUSTOM', '--foo', 'bar', parserMap);
    ...
    describe('with invalid type', () => {
        it('throws TypeError when an invalid type is provided', () => {
            expect(() => parseArgumentWithParam('--foo=bar', 'INVALID_TYPE' as any, '--foo', 'bar', undefined)).toThrow(TypeError);
        });
    });
